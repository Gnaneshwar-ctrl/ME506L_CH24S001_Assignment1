...
for n = n_points
    ...
    spline_error = abs(y_exact - spline_interp);
    
    figure;
    plot(x_exact, newton_error, 'r-', 'LineWidth', 2); hold on;
    plot(x_exact, spline_error, 'b--', 'LineWidth', 2);
    xlabel('x');
    ylabel('Error');
    title(['Error Comparison (Newton vs Cubic Spline, n = ', num2str(n), ')']);
    legend('Newton Error', 'Cubic Spline Error', 'Location', 'Best');
    grid on;
end
